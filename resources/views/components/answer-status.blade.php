...
@if ($questions[$q]['is_correct'] === false)
    <div class="p-4 text-red-800 border border-red-300 rounded-lg bg-red-50" x-data="{ revealed: false }">
        ...
        <div class="mt-2 mb-4 text-sm">
            Správna odpoveď je: <span class="font-bold transition-all duration-300 ease-in-out"
                                      :class="revealed ? '' : 'blur-sm select-none'">{{ $questions[$q]['answer'] }}</span>.
            Ak chceš, tak si môžes pozrieť video vysvetlujúce túto úlohu alebo
            môžes odhaliť rozostretú odpoveď.
        </div>
        <div class="flex">
            <a href="{{ $questions[$q]['explanation'] }}" target="_blank"
